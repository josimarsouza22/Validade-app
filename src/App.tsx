...
export default function App() {
  ...
  const handleAddProduct = async (e: FormEvent) => {
    ...
    if (editingProduct) {
      ...
      try {
        ...
        alert('Erro ao atualizar no banco de dados: Permissão Negada ou problema de conexão.');
      }
      ...
      try {
        ...
        alert('Erro ao salvar no banco de dados: Permissão Negada ou problema de conexão.');
      }
    }
  ...
  const removeProduct = async (id: string) => {
    try {
      ...
      alert('Erro ao excluir no banco de dados: Permissão Negada ou problema de conexão.');
    }
  };
  ...
  const removeExpiredProducts = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      alert('Erro ao limpar vencidos no banco de dados: Permissão Negada ou problema de conexão.');
    } finally {
      setIsDeleting(false);
      ...
      <div className="max-w-6xl mx-auto">
            ...
            {user ? (
              <>
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-slate-800 hover:bg-slate-900 text-white px-4 py-2 rounded-xl font-medium transition-colors shadow-sm ml-2"
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2 bg-slate-200/50 p-1 rounded-xl w-fit">
            <button
              onClick={() => setFilterStatus('all')}
              className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${filterStatus === 'all'
            ...
            <button
              onClick={() => setFilterStatus('expired')}
              className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${filterStatus === 'expired'
            ...
            <button
              onClick={() => setFilterStatus('near')}
              className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${filterStatus === 'near'
            ...
            <button
              onClick={() => setFilterStatus('on-track')}
              className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${filterStatus === 'on-track'
          ...
          {(user && products.some(p => calculateDaysRemaining(p.expirationDate) <= 0)) && (
            <button
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-bottom border-slate-200">
                  ...
                  {user && <th className="px-6 py-4 text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Ações</th>}
                </tr>
              </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold ${isExpired
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 border-b border-slate-100 flex flex-col items-center">
                ...
                <p className="text-sm text-slate-500 mt-1 text-center">Entre com suas credenciais para gerenciar validades.</p>
              </div>
              ...
              <form onSubmit={handleLogin} className="p-6 space-y-4">
                ...
                <div className="pt-2 flex justify-end gap-3">
                  ...
                  <button
                    ...
                    className={`bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-xl font-medium transition-colors shadow-sm flex items-center gap-2 ${isLoggingIn ? 'opacity-70 cursor-not-allowed' : ''}`}
                  >
